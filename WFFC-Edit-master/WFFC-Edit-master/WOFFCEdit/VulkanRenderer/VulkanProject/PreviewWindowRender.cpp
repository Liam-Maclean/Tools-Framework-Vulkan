#include "PreviewWindowRender.h"




void PreviewWindowRender::InitialiseVulkanApplication()
{
	PrepareScene();
	...
	PreviewWindowRender::CreateCamera();
	PreviewWindowRender::_CreateGeometry();
	PreviewWindowRender::CreateGBuffer();
	PreviewWindowRender::SetUpUniformBuffers();
	PreviewWindowRender::_CreateDescriptorSetLayout();
	PreviewWindowRender::_CreateVertexDescriptions();
	PreviewWindowRender::_CreateGraphicsPipeline();
	PreviewWindowRender::_CreateDescriptorPool();
	PreviewWindowRender::_CreateDescriptorSets();
	PreviewWindowRender::_CreateCommandBuffers();
	PreviewWindowRender::CreateDeferredCommandBuffers();
	PreviewWindowRender::Update();
}

PreviewWindowRender::~PreviewWindowRender()
{
...
void PreviewWindowRender::Update()
{
#ifdef WIN32
...
void PreviewWindowRender::CreateCamera()
{
	//Creates the camera with camera values to create view and projection matrices 
	camera = new Camera(glm::vec3(0.0, 2.0f, 4.0f), glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 60.0f, glm::vec2(_swapChainExtent.width, _swapChainExtent.height), 0.1f, 100.0f);
}

void PreviewWindowRender::DrawFrame()
{
	uint32_t imageIndex;
...
void PreviewWindowRender::_CreateGraphicsPipeline()
{
...
void PreviewWindowRender::UpdateUniformBuffer(uint32_t currentImage)
{
...
VkPipelineShaderStageCreateInfo PreviewWindowRender::loadShader(std::string fileName, VkShaderStageFlagBits stage)
{
...
void PreviewWindowRender::_CreateGeometry()
{
...
void PreviewWindowRender::_CreateRenderPass()
{
	VulkanWindow::_CreateRenderPass();
}

void PreviewWindowRender::_CreateShaderBuffer(VkDevice device, VkDeviceSize size, VkBuffer * vertexBuffer, VkDeviceMemory * vertexBufferMemory, VkBufferUsageFlagBits bufferStage, void * data)
{
	...
	_CreateBuffer(bufferSize, bufferStage, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, *vertexBuffer, *vertexBufferMemory);
...
void PreviewWindowRender::SetUpUniformBuffers()
{
...
void PreviewWindowRender::CreateGBuffer()
{
...
void PreviewWindowRender::_CreateDescriptorSets()
{
...
void PreviewWindowRender::_CreateDescriptorSetLayout()
{
	//====Deferred Layouts====
...
void PreviewWindowRender::CreateDeferredCommandBuffers()
{
...
void PreviewWindowRender::_CreateCommandBuffers()
{
...
void PreviewWindowRender::_CreateVertexDescriptions()
{
...
void PreviewWindowRender::_CreateDescriptorPool()
{
